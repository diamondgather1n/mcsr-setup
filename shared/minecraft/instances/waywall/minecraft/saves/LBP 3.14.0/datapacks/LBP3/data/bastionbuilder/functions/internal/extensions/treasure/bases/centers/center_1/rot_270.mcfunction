...
execute if score id_piglins bastion.settings matches 1.. positioned ~3 ~3 ~-5 as @e[type=piglin, tag=bastion_mob, dx=1, dy=0, dz=0] run data modify entity @s CustomName set value '{"text":"bases/centers/center_1","color":"yellow"}'