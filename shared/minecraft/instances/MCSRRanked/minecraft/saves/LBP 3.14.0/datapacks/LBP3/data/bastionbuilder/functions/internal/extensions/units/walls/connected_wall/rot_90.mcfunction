...
execute if score id_piglins bastion.settings matches 1.. positioned ~-10 ~5 ~2 as @e[type=piglin, tag=bastion_mob, dx=4, dy=9, dz=10] run data modify entity @s CustomName set value '{"text":"walls/connected_wall","color":"yellow"}'