...
execute if score id_piglins bastion.settings matches 1.. positioned ~-8 ~1 ~11 as @e[type=piglin, tag=bastion_mob, dx=4, dy=28, dz=2] run data modify entity @s CustomName set value '{"text":"ramparts/mid_wall_main","color":"yellow"}'