...
execute if score id_piglins bastion.settings matches 1.. positioned ~15 ~25 ~-11 as @e[type=piglin, tag=bastion_mob, dx=4, dy=1, dz=9] run data modify entity @s CustomName set value '{"text":"starting_pieces/entrance_base","color":"yellow"}'