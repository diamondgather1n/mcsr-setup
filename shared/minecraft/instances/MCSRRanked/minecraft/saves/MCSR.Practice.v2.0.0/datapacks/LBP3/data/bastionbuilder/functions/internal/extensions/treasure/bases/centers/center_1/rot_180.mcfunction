...
execute if score id_piglins bastion.settings matches 1.. positioned ~-5 ~3 ~-4 as @e[type=piglin, tag=bastion_mob, dx=0, dy=0, dz=1] run data modify entity @s CustomName set value '{"text":"bases/centers/center_1","color":"yellow"}'