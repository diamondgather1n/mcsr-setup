...
mapfile -t lag < <(read_list "$ROOT/packages/pacman-x11-lag.txt")
sudo pacman -Syu --needed "${common[@]}" "${x11[@]}" "${lag[@]}"
...
install -Dm644 "$ROOT/x11/i3/L-config" "$HOME/.config/i3/config"
...
for f in "$ROOT/x11/helpers/"* "$ROOT/shared/scripts/"*; do
...
echo "Installed L X11 setup. It does not make X11 the active session."
