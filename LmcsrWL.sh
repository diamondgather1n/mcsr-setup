...
mapfile -t lag < <(read_list "$ROOT/packages/pacman-wayland-lag.txt")
sudo pacman -Syu --needed "${common[@]}" "${wayland[@]}" "${lag[@]}"
...
install -Dm644 "$ROOT/wayland/jay/L-config.toml" "$HOME/.config/jay/config.toml"
install -Dm644 "$ROOT/wayland/waywall/L-init.lua" "$HOME/.config/waywall/init.lua"
mkdir -p "$HOME/.config/waywall/resources"
cp -a "$ROOT/wayland/waywall/resources/." "$HOME/.config/waywall/resources/"
install -Dm644 "$ROOT/wayland/waybar/config" "$HOME/.config/waybar/config"
install -Dm644 "$ROOT/wayland/waybar/style.css" "$HOME/.config/waybar/style.css"
install -Dm644 "$ROOT/x11/i3blocks/config" "$HOME/.config/i3blocks/config"
for f in "$ROOT/x11/helpers/bar-cpu" "$ROOT/x11/helpers/bar-gpu" "$ROOT/x11/helpers/bar-net" "$ROOT/x11/helpers/bar-volume"; do
...
for f in "$ROOT/wayland/helpers/"* "$ROOT/shared/scripts/"*; do
...
install -Dm755 "$ROOT/shared/obs/input-overlay/obs-input-overlay" "$HOME/.local/bin/obs-input-overlay"
install -Dm644 "$ROOT/shared/obs/input-overlay/obs-input-overlay.service" "$HOME/.config/systemd/user/obs-input-overlay.service"
systemctl --user enable --now obs-input-overlay.service
sudo systemctl enable lightdm.service

echo "Installed L Wayland setup with vendored Jay 1.14.0; see packages/sources.md for source provenance."
